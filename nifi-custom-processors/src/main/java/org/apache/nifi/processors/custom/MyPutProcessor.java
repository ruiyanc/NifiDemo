...
public class MyPutProcessor extends AbstractProcessor {

    Logger log = Logger.getLogger(String.valueOf(MyPutProcessor.class));
...
//
////                sendKafka(filepath,"nifitest");
////                String result = "处理了：" + context.getProperty("MY_PROPERTY").getValue();
...
//        linkedQueue.offer(System.currentTimeMillis() - startTime);
...
//        }
    ...
    public AtomicInteger sendKafka(String monitoringPath,String topic) {
        //判断结构化发送kafka和非结构化发送
        //读取文件内容
        try {
...
//                }
    ...
    public void insertLog(String path) throws IOException {
//        String time = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
//        String date = time.substring(0, 8);
        String logPath = "/HTData/get.log";
        File logFile = new File(logPath);
        if (!logFile.exists()) {
            logFile.createNewFile();
        }
        BufferedWriter writer = new BufferedWriter(new FileWriter(logFile, true));
        writer.write(path + "," + System.currentTimeMillis() + "\r\n");
